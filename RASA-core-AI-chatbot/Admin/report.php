...
$sections_selected = isset($_GET['sections']) && is_array($_GET['sections']) && !empty($_GET['sections'])
    ? array_filter($_GET['sections'], function($section) use ($default_sections) {
        return in_array($section, $default_sections);
    })
    : $default_sections;
...
$intents_current = fetchData($conn, "SELECT i.intent_name, COUNT(*) as count FROM chatlogs cl JOIN intent_examples ie ON cl.question LIKE CONCAT('%', ie.example_text, '%') JOIN intents i ON ie.intent_id = i.id WHERE cl.$date_sql GROUP BY i.intent_name ORDER BY count DESC LIMIT 5");
...
$total_queries_current = count(fetchData($conn, "SELECT * FROM chatlogs WHERE $date_sql"));
...
$avg_response_time_current = fetchData($conn, "SELECT AVG(TIMESTAMPDIFF(SECOND, timestamp, responded_timestamp)) as avg_time FROM responded_query WHERE $date_sql")[0]['avg_time'] ?? 0;
$peak_usage_current = $daily_queries_current ? max(array_column($daily_queries_current, 'queries')) : 0;
$avg_session_duration = fetchData($conn, "SELECT AVG(session_duration) as avg_duration FROM user WHERE session_id IN (SELECT DISTINCT session_id FROM chatlogs WHERE $date_sql)")[0]['avg_duration'] ?? 0;
            ...
            <div class="sb2-2 col-md-9">
                ...
                <div class="sb2-2-3">
                    ...
                    <div class="report-header">
                        ...
                        <div class="meta">
                            <span>Generated by: <b><?php echo htmlspecialchars($admin['username'] ?? 'Unknown Admin'); ?></b></span>
                            <span>Admin id: <b><?php echo htmlspecialchars($admin['admin_id'] ?? 'Unknown'); ?></b></span>
                        ...
                        <div style="text-align:center;">
                            <span class="date-range-badge">
                                ...
                                switch ($date_range) {
                                    ...
                                    case 'custom': $date_label = "Custom: $from to $to"; break;
                                }
                                echo htmlspecialchars($date_label);
                                ?>
                            </span>
                        </div>
                    </div>
                    ...
                    <div style="text-align:center; margin-top: 16px; margin-bottom: 10px;">
                        <form id="pdfExportForm" method="POST" action="report.php" style="display:inline;">
                    ...
                    <div class="report-filter-section" style="margin-top: 70px;">
                        <?php
                        $filter_error = '';
                        if ($date_range === 'custom' && (empty($from) || empty($to))) {
                        ...
                        <form method="GET" style="margin-bottom: 0;">
                            <div class="report-filter-row">
                                <div class="report-filter-col">
                                    ...
                                    <select name="date_range" id="date_range" onchange="toggleCustomDates(this.value)">
                                ...
                                <div class="report-filter-col" id="custom_dates" style="display: <?php echo $date_range === 'custom' ? 'block' : 'none'; ?>;">
                                    <?php if ($filter_error): ?>
                                        <div style="color: red; font-weight: bold; margin-bottom: 8px;"><?php echo htmlspecialchars($filter_error); ?></div>
                                    <?php endif; ?>
                                    <label>From:</label>
                                    <input type="date" name="from" class="report-filter-input" value="<?php echo htmlspecialchars($from); ?>">
                                    <label>To:</label>
                                    <input type="date" name="to" class="report-filter-input" value="<?php echo htmlspecialchars($to); ?>">
                                </div>
                                <div class="report-filter-col">
                    ...
                    <div class="container" style="width: 100%;">
                        ...
                        <?php if (empty($users_current) && empty($chatlogs_current) && empty($feedback_current) && empty($faq_frequency_current) && empty($pushed_query_current) && empty($daily_queries_current) && empty($hourly_queries_current) && empty($intents_current)): ?>
                            <div style="text-align:center; color:#c82333; font-weight:bold; margin:2em 0;">No data found for the selected filter(s).</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            filterForm.addEventListener('submit', function(e) {
                const checkboxes = filterForm.querySelectorAll('input[name="sections[]"]');
            ...
            document.getElementById('pdfExportForm').addEventListener('submit', function(e) {
                const chart1 = document.getElementById('dailyQueryVolumeChart');
                const chart2 = document.getElementById('hourlyQueryChart');
                document.getElementById('chart1_img').value = chart1 ? chart1.toDataURL('image/png') : '';
                document.getElementById('chart2_img').value = chart2 ? chart2.toDataURL('image/png') : '';
            });
        });
